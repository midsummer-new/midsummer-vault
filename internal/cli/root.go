...
var rootCmd = &cobra.Command{
	Use:   "vault",
	Short: "Midsummer Vault — secret management for AI agents",
...
%sSecrets:%s
  ...
  list          List secret names          vault list [-v]
  import        Bulk import from .env      vault import .env.local
  describe      Set description            vault describe KEY "desc"
  rotate        Mark secret as rotated     vault rotate KEY
...
%sRemote:%s %s(optional — for teams)%s
  ...
  sync push     Push encrypted blob        vault sync push --env production
  sync pull     Pull encrypted blob        vault sync pull --env staging
...
func init() {
	rootCmd.Version = "midsummer-vault " + Version
